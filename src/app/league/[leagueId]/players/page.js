      ...
      if (isForeigner) {
        ...
        if (isTargetActive) {
          if (activeCount + 1 > activeLimit) {
            // Violation of Active Limit
            // If On-Team is not violated, but Active IS violated, we must prevent Add or force Drop Active?
            // Usually user needs to drop an Active foreigner.
            violation = `Foreigner Active Limit Exceeded (Limit: ${activeLimit})`;
            vType = 'foreigner_limit'; // Same type for drop filtering
            console.log(`[PreCheck] VIOLATION: ${violation}`);
          } else {
            ...
            <div className="p-6">
              ...
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button
                  ...
                  disabled={!dropCandidateID || isAdding}
                  className={`px-6 py-2 rounded-xl font-bold shadow-lg transition-all ${!dropCandidateID || isAdding ? 'bg-slate-700 text-slate-500 cursor-not-allowed' : 'bg-gradient-to-r from-red-600 to-red-500 text-white hover:scale-105'}`}
                >
                  {isAdding ? 'Processing...' : 'Confirm Add & Drop'}
                </button>
              </div>
            </div>
          </div>
